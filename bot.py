...
# ============================================
# КНОПКИ СЛОЖНОСТИ ДЛЯ КАЖДОГО РЕЙДА
# ============================================

class RaidDifficultyView(ui.View):
    ...
    @ui.button(label="🇳 Обычный", style=discord.ButtonStyle.green, row=0)
    ...
    @ui.button(label="🇭 Героический", style=discord.ButtonStyle.red, row=0)
...
async def setup_menu_auto():
    ...
    for guild in bot.guilds:
        ...
        embed_header = discord.Embed(
            ...
            description="Выберите рейд и сложность:",
            color=discord.Color.gold()
        )
        embed_header.set_footer(text="Для создания лобби нажмите на кнопки ниже")
        await channel.send(embed=embed_header)

        # Создаем сообщение для каждого рейда
        for raid_key, raid_config in RAID_CONFIG.items():
            role = discord.utils.get(guild.roles, name=raid_config['role'])
            embed = discord.Embed(
                title=f"{raid_config['emoji']} {raid_config['name']}",
                description=f"Роль: {role.mention if role else 'роль не найдена'}",
                color=raid_config['color']
            )
            view = RaidDifficultyView(raid_key)
...
async def setup_menu(interaction: discord.Interaction):
    ...
    await interaction.channel.send(embed=embed_header)

    for raid_key, raid_config in RAID_CONFIG.items():
        role = discord.utils.get(interaction.guild.roles, name=raid_config['role'])
        embed = discord.Embed(
            title=f"{raid_config['emoji']} {raid_config['name']}",
            description=f"Роль: {role.mention if role else 'роль не найдена'}",
            color=raid_config['color']
